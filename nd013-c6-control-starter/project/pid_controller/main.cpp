...
//// Step 1: PID controller for steer:
///The PID parameters(Kp,Ki,Kd) are tuned to get satisfying results and the results will be explained under below.
const double KP_STEER{0.5};
const double KI_STEER{0.0005};
const double KD_STEER{0.0007};
const double MAX_STEER{1.2};
const double MIN_STEER{-1.2};

//// Step 2: PID controller for throttle:
//the throttle error for the throttle PID controller is computed based on the speed difference between the actual speed and the desired speed.
const double KP_THROTTLE{0.2};
const double KI_THROTTLE{0.0005};
const double KD_THROTTLE{0.001};
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          **/
          ...
          //// If error_throttle > 0, it means actual speed < desired speed so it is needed to accelerate,and if error_throttle < 0, it means actual speed > desired speed so it is needed to brake.
